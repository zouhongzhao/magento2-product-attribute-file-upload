<?php
namespace Iggo\AdvancedBundles\Helper;
...
class Data extends \Magento\Framework\App\Helper\AbstractHelper{
    ...
    public function getSelectionOptionQty($selectionQty){
        $result = array();
        if($selectionQty){
            $selectionQty = @json_decode($selectionQty,true);
            if(isset($selectionQty['selection_qty'])){
                foreach ($selectionQty['selection_qty'] as $row) {
                    $result[(string)$row['name']] = (int)$row['qty'];
                }
            }
        }
        // if(!$result){
